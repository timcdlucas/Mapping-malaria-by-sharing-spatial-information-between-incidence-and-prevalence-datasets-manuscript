...
Firstly, especially when modelling at continental scales \cite{lancet}, there are often some areas with better coverage of prevalence surveys than polygon incidence data.
...
Finally, estimates of both prevalence and incidence metrics are useful for policy makers \cite{cohen2017mapping} but typically models are fitted to observations of one metric and then a secondary model is used to convert to the other \emph{post hoc} \cite{battlelancet, bhatt2015}.
...
The polygon incidence data were collated from various government reports and adjusted to a standard metric using methods defined in \cite{cibulskis2011worldwide, weiss2019}.
...
Finally a temperature suitability mask was used to created rasters of population at risk \cite{}. % todo
...
This model is the same as the baseline disaggregation model except that it has one extra covariate.
...
\caption{{\bf Input incidence data and predicted incidence maps. } 
The incidence (log10) data, predicted log10 incidence from the prevalence GP model for spatially sampled out-of-sample polygons (middle) and predicted log10 incidence from the joint model for spatially sampled out-of-sample polygons (bottom) for Senegal.
}
\label{predobsmapsen}
\end{figure}
