...
For each country we fitted a binomial likelihood, hierarchical Gaussian random field with the same hyperpriors as above.
These models were fitted using R-INLA \cite{INLA}.
To be in the correct scale for the dissagregation model, the inverse logit of the predicted Gaussian field (i.e. the liver predictor of the model) was used for the additional covariate.
...
The polygon data are assigned a Poisson likelihood as before but the prevalence point-survey data, $z_b$, is given a binomial likelihood
...
Finally, the second iid random effect, $w_i \sim \operatorname{Norm}(0, \sigma_w)$, was applied to each point survey.
