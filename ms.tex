...
One major barrier to this modelling approved us that these two data sources are on different scales: point surveys are a measurement of prevalence $\[0, 1\]$ while routine surveillance measures incidence $\[0, \infty\]$.
...
where $\mathrm{p2i}$ is a from a model that was published previously. \cite{cameron2015defining}
...
The Gaussian spatial effect $u(s, \tau, \kappa)$ has a Mat\'ern covariance function and two hyper parameters: $\rho$, the nominal range (behind which correlation is $< 0.1$) and $\sigma_u$, the marginal standard deviation.
The first iid random effect, $v_j \sim \operatorname{Norm}(0, \sigma_v)$,  is grouped by polygon, with all pixels and point surveys within polygon $j$ being grouped together.
This random effect models both missing covariates and extra-Poisson sampling error. 
The second iid random effect, $w_i \sim \operatorname{Norm}(0, \sigma_w)$, is applied to each point survey.
This effect models extra-binomial sampling noise.
...
We assign $\rho$ and $\sigma_u$ a joint penalised complexity prior \ref{notSimpson} such that $P(\rho < 3) = 0.00001$ and $P(\sigma_u > 1) = 0.00001$.
We believe that a large proportion of the variance of malaria prevalence and incidence cannot be explained by a linear combination of the covariates selected, so we set this prior such that the girls could explain most of the range of the data.

We assign \rho$ and $\sigma_u$ penalised complexity priors as well such that $P(\sigma_v > 0.05) = 0.00001$ and $P(\sigma_w > 0.05) = 0.00001$. 
The former is based on a comparison of the variance of Poisson random variables, with rates given by the number of cases observed, and an independently derived upper and lower bound for three case counts \ref{cibulskis}.
We found that an iid effect with a standard deviation of 0.05 would be able to account for the discrepancy.
...
\begin{minipage}{0.45\textwidth} \centering 
\includegraphics[width = 0.9\textwidth]{figures/idn_cv_spatial.png} \caption{Indonesia random crossvalidation} 
\end{minipage}
...
\centering
\caption{
{\bf Summary of out-of-sample accuracy.}}
\begin{tabular}{l|l|l|l|l}
\hline
{\bf Holdout data} & {\bf Metric} & {\bf Country} &  {\bf Points} & {\bf Polygons} & {\bf Joint} \\
\thickhline \\
\multirow{6}{Incidence} & \multirow{3}{Pearson} & Indonesia & 0.1 &. 0.2 & {\bf 0.3}\\
                   & Madagascar & 0.1 &. 0.2 & {\bf 0.3}\\
                   & Senagal & 0.1 &. 0.2 & {\bf 0.3}\\
                                          & \multirow{3}{Spearman} & Indonesia & 0.1 &. 0.2 & {\bf 0.3}\\
                   & Madagascar & 0.1 &. 0.2 & {\bf 0.3}\\
                   & Senagal & 0.1 &. 0.2 & {\bf 0.3}\\
\multirow{6}{Prevalence} & \multirow{3}{Pearson} & Indonesia & 0.1 &. 0.2 & {\bf 0.3}\\
                   & Madagascar & 0.1 &. 0.2 & {\bf 0.3}\\
                   & Senagal & 0.1 &. 0.2 & {\bf 0.3}\\
                                         & \multirow{3}{Spearman} & Indonesia & 0.1 &. 0.2 & {\bf 0.3}\\
                   & Madagascar & 0.1 &. 0.2 & {\bf 0.3}\\
